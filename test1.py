...
# DataFrame to store all reservations
...
def pilih_kursi(jumlah_tiket, kd_seat, tanggal, tujuan):
    ...
    # Check for previously reserved seats for the same date and destination
    reserved_seats = df_reservations[
        (df_reservations['Tanggal Perjalanan'] == tanggal) & 
        (df_reservations['Tujuan'] == tujuan)
    ]['Kursi yang Dipilih'].apply(lambda x: list(map(int, x.split(', ')))).sum()
